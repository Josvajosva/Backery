...
patch(PosStore.prototype, {
    ...
    async _getCurrentStock(productId) {
        try {
            const lots = await this.env.services.orm.call(
                'stock.lot',
                'search_read',
                [
                    [['product_id', '=', productId], ['removal_date', '!=', false], ['product_qty', '>', 0]],
                    ['id', 'name', 'product_qty', 'removal_date', 'expiration_date']
                ],
                {},
                { shadow: true }
            );
            
            const res =  new Map(lots.map(lot => [lot.id, {
                id: lot.id,
                name: lot.name,
    ...
    async addLineToOrder(vals, order, opts = {}, configure = true) {
        ...
        if (values.product_id.isTracked() && (configure || code)) {
            ...
            // Get current stock from the backend to ensure fresh data
            const currentStock = await this._getCurrentStock(productId);
            this.stock_lot = currentStock; // Update the local stock_lot with fresh data

            const allLots = Array.from(this.stock_lot.values())
                .filter(lot => 
                    lot.product_id && 
                    lot.product_id.id === productId && 
                    lot.removal_date
                );
            ...
            if (allLots.length > 0) {
                ...
                const sortedByDate = [...allLots]
                    .filter(lot => {
                        const removalDate = new Date(lot.removal_date);
                        removalDate.setHours(0, 0, 0, 0); // Normalize removal date
                        return removalDate >= today; // Includes today and future dates
                    })
                    .sort((a, b) => {
                        // Sort by nearest removal date (today first, then future dates)
                        const dateA = new Date(a.removal_date);
                        const dateB = new Date(b.removal_date);
                        return dateA - dateB;
                    });
                
                const lotQuantities = {};
                allLots.forEach(lot => {
                    lotQuantities[lot.id] = lot.product_qty; 
                });

                Array.from(order.lines.values())
                    .filter(line => line.product_id.id === productId)
                    .forEach(line => {
                        if (line.pack_lot_ids && line.pack_lot_ids.length > 0) {
                            line.pack_lot_ids.forEach(lot => {
                                const lotId = lot.raw.lot_id;
                                if (lotId && lotQuantities[lotId] !== undefined) {
                                    lotQuantities[lotId] -= line.qty;
                                }
                            });
                        } else {
                            for (const lot of sortedByDate) {
                                if (lotQuantities[lot.id] > 0) {
                                    const deductAmount = Math.min(line.qty, lotQuantities[lot.id]);
                                    lotQuantities[lot.id] -= deductAmount;
                                    if (deductAmount === line.qty) break;
                                }
                            }
                        }
                    });
                ...
                // First try to find a lot with today's removal date
                const todaysLot = sortedByDate.find(lot => {
                    const removalDate = new Date(lot.removal_date);
                    removalDate.setHours(0, 0, 0, 0);
                    return removalDate.getTime() === today.getTime() && lotQuantities[lot.id] > 0;
                });
                
                if (todaysLot) {
                    selectedLot = todaysLot;
                } else {
                    // If no today's lot available, find the nearest future lot
                    for (const lot of sortedByDate) {
                        if (lotQuantities[lot.id] > 0) {
                            selectedLot = lot;
                            break;
                ...
                if (selectedLot) {
                    ...
                    console.warn("No available lots for product", productId);
                }
            }
        }
